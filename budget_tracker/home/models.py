...
class FinancialTrend(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='financial_trends')
    prediction_date = models.DateField()  # Future date for which prediction is made
    predicted_income = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    predicted_expense = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Trend on {self.prediction_date} for {self.user.email}"

class FinancialRecommendation(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='financial_recommendations')
    recommendation = models.TextField()
    generated_on = models.DateTimeField(auto_now_add=True)
    related_to_date = models.DateField(null=True, blank=True)  # Optional: Which date this rec is about
    is_applied = models.BooleanField(default=False)

    def __str__(self):
        return f"Recommendation for {self.user.email} on {self.generated_on.date()}"
