...
from .models import FinancialTrend, FinancialRecommendation
...
def financial_trends(request):
    ...
    for i in range(days):
        ...
        daily_income = Income.objects.filter(user=user, date=target_date).aggregate(total=Sum('amount'))['total'] or 0
        daily_expense = Expense.objects.filter(user=user, date=target_date).aggregate(total=Sum('amount'))['total'] or 0

        trends.append({
            'prediction_date': target_date,
            'predicted_income': daily_income,
            'predicted_expense': daily_expense,
            'created_at': today
        })

    trends = sorted(trends, key=lambda x: x['prediction_date'], reverse=True)

    return render(request, 'home/financial_trends.html', {'trends': trends})
...
def financial_recommendations(request):
    ...
    existing_recommendation = FinancialRecommendation.objects.filter(user=user, generated_on=today).first()

    if not existing_recommendation:
        # Example rule: if total expenses in past 7 days > 80% of income, suggest reducing
        from datetime import timedelta
        last_week = today - timedelta(days=7)

        income_total = Income.objects.filter(user=user, date__gte=last_week).aggregate(total=Sum('amount'))['total'] or 0
        expense_total = Expense.objects.filter(user=user, date__gte=last_week).aggregate(total=Sum('amount'))['total'] or 0

        if income_total and expense_total > Decimal('0.8') * income_total:
            rec_text = "⚠️ Your expenses are more than 80% of your income this week. Consider reducing discretionary spending."
        else:
            rec_text = "✅ You're doing well this week! Keep monitoring your budget regularly."
    ...
    recommendations = FinancialRecommendation.objects.filter(user=user).order_by('-generated_on')
    return render(request, 'home/financial_recommendations.html', {'recommendations': recommendations})
